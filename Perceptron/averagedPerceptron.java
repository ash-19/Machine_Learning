...
public class averagedPerceptron {
	...
	private static Double[] simplePerceptron4Fold(double lRate) {
		Double[] w = getRandomW();
		Double[] a = new Double[DIMENSIONALITY - 1];
		Arrays.fill(a, 0.0);
		...
		for(int i = 0; i < 10; i++) {
			...
			for(Double[] entry : fourFoldTrainData) {
				...
				aVector = aVector.plus(wVector);
				avgBias = avgBias + bias;
			}
		}
		
		return aVector.getDoubleArray();
	}
